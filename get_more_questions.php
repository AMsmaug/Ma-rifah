...
$start = $inputs->start ?? 0; // Default to 0 if not provided
$limit = $inputs->limit ?? 10; // Default to 10 if not provided
$student_id = $inputs->studentId ?? 3;
...
if ($student_id != null) {
    ...
    WHERE
        rq.rank > $start AND rq.rank <= $start + $limit
    GROUP BY
